        ...
        for i in range(self.block_x):
            for j in range(self.block_y):
                if target_blocks[i][j].points != 0:
                    mean_base, std_base = self.point2point_mean_and_std_deviation(source_blocks[i][j],
                                                                                 baseline_block[i][j])
                    mean_target, std_target = self.point2point_mean_and_std_deviation(source_blocks[i][j],
                                                                                     target_blocks[i][j])
